...
const htmlGroup = (node: AltGroupNode, isJsx: boolean = false): string => {
  ...
  if (builder.style) {
    ...
    return `\n<div${attr} id="${node.id}">${indentString(generator)}\n</div>`;
  }
...
export const htmlBuilder = (
  ...
  const builderResult = new HtmlTextBuilder(node, showLayerName, isJsx)
    ...
    .textTransform(node);
  console.log('xxxxxHTMLBUILDER', node.id, builderResult.style)
...
const htmlText = (
  ...
  if (isInput) {
    ...
    return `\n<p${builder.build()} id="${node.id}">${charsWithLineBreak}</p>`;
  }
};
...
export const htmlContainer = (
  ...
  const builder = new HtmlDefaultBuilder(node, showLayerName, isJsx)
    ...
    .shadow(node)
    .border(node);
  console.log('xxxxxHTMLBUILDER', node.id, builder.style)
  ...
  if (builder.style || additionalStyle) {
    ...
    console.log('xxxBuild', node.id, build)
    ...
    if (children) {
      return `\n<${tag}${build}${src} id="${node.id}">${indentString(children)}\n</${tag}>`;
    } else if (selfClosingTags.includes(tag) || isJsx) {
      return `\n<${tag}${build}${src} id="${node.id}"/>`;
    } else {
      return `\n<${tag}${build}${src} id="${node.id}"></${tag}>`;
    }
  }
