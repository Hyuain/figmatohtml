...
export class HtmlDefaultBuilder {
  ...
  position(
    ...
    if (position === "absoluteManualLayout" && node.parent) {
      ...
      console.log('xxxALeft', node.id, left, top, node.x, node.y, parentX, parentY)
      ...
      console.log('xxxALeft', node.id, position)
      this.style += position;
    }
    return this;
  }
  ...
  widthHeight(node: AltSceneNode): this {
    ...
    console.log('xxxWidthHeight', this.style)
    return this;
  }
  ...
  overflowHidden(node: any): this {
    if (node.clipsContent) {
      this.style += formatWithJSX("overflow", this.isJSX, 'hidden');
    }
    return this
  }
