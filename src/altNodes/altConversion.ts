...
export const convertIntoAltNodes = (
  ...
  const mapped: Array<AltSceneNode | null> = sceneNode.map(
    (node: SceneNode) => {
      console.log('xxxconverting', node)
...
const convertLayout = (altNode: AltLayoutMixin, node: LayoutMixin) => {
  ...
  //   altNode.y = node.y;
  // }
  altNode.x = node.absoluteBoundingBox.x;
  altNode.y = node.absoluteBoundingBox.y;

  altNode.width = node.absoluteBoundingBox.width;
  altNode.height = node.absoluteBoundingBox.height;
...
const convertFrame = (altNode: AltFrameMixin, node: DefaultFrameMixin) => {
  altNode.layoutMode = node.layoutMode || 'NONE';
